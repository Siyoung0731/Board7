	...
	<main>
		...
		<h2 class="h2"><b id="mname">${menu_name}</b>게시글 내용 보기</h2>
		  <table id="table1">
		    ...
		    <tr>
		      <td colspan="4">
		        <a class="btn btn-light" href="/BoardPaging/WriteForm?menu_id=${menu_id}&nowpage=${nowpage}">새글쓰기</a>
		        <%-- <c:if test="${sessionScope.login.userid eq board.writer }"> --%>
		        
		        <a class="btn btn-light" href="/BoardPaging/UpdateForm?idx=${board.idx}&menu_id=${board.menu_id}&nowpage=${nowpage}">수정</a>
		        <a class="btn btn-light" href="/BoardPaging/Delete?idx=${board.idx}&menu_id=${board.menu_id}&nowpage=${nowpage}">삭제</a>		        
		        <%-- </c:if> --%>
		              
		        <a class="btn btn-light" href="/BoardPaging/List?menu_id=${menu_id}&nowpage=${nowpage}">목록</a>		      
